"""Descarga M1 de Dukascopy (bi5) CONCURRENTE para un simbolo, ultimos N meses,
resamplea a M5 con MID=(bid+ask)/2 -> CSV del proyecto.
Concurrencia: el cuello es latencia (14s/archivo), no banda. Con 16 threads
576 archivos/par bajan de ~2.5h a ~10 min.
"""
...
def download(sym):
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as ex:
        ...
        for fut in as_completed(futs):
            ...
            if done % 100 == 0:
                print(f"  [{sym}] {done}/{len(jobs)} archivos", flush=True)
    ...
    for i in range(0, len(m1), 5):
        ...
        t0 = g[0][0]
        o = g[0][1]; c = g[-1][4]
        hi = max(r[2] for r in g); lo = min(r[3] for r in g)
        v = sum(r[5] for r in g)
...
if __name__ == "__main__":
    for sym in SYMS:
        print(f"Descargando {sym} ({MONTHS} meses, {WORKERS} workers)...")
        fn, n = download(sym)
